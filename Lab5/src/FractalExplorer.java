...
public class FractalExplorer 
{
    ...
    {
    	JFrame frame = new JFrame("Рисунок фракталом");
    	JButton buttonReset = new JButton("Сбросить изображение");
        ...
        buttonReset.setActionCommand("Сбросить изображение");
        buttonReset.addActionListener(new ActionHandler());
        
        buttonSave.setActionCommand("Сохранить изображение");
    ...
    private void drawFractal()
    {
    	for (int x = 0; x < sizeDisplay; x++)
    	{
    		...
    		{
    			int count = fractalGenerator.numIterations(FractalGenerator.getCoord(range.x, range.x + range.width, sizeDisplay, x) , FractalGenerator.getCoord(range.y, range.y + range.width, sizeDisplay, y));
    			if (count == -1)
    			{
    ...
    public class ActionHandler implements ActionListener 
    {
        ...
        {
            if (e.getActionCommand().equals("Сбросить изображение")) 
            {
                fractalGenerator.getInitialRange(range);
                drawFractal();
            } 
            else if (e.getActionCommand().equals("Сохранить изображение")) 
            {
                ...
                if (t == JFileChooser.APPROVE_OPTION) 
                {
                    ...
                    {
                        JOptionPane.showMessageDialog(imageDisplay, ee.getMessage(), "Невозможно сохранить изображение", JOptionPane.ERROR_MESSAGE);
                    }
                }
            } 
            else 
